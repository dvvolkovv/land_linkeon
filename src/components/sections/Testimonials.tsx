  ...
  return (
    <Section ariaLabelledby="testimonials-heading">
      <FadeIn className="text-center mb-16 max-w-2xl mx-auto">
        ...
        <h2 id="testimonials-heading" className="text-4xl md:text-5xl font-semibold tracking-tight text-gray-900 text-balance">
          {t('testimonials.h2')}
        </h2>
      </FadeIn>
        ...
        {(Array.isArray(items) ? items : []).map((it, i) => (
          <FadeIn key={i} delay={i * 100}>
            <Card className="p-6 h-full flex flex-col">
              <Quote aria-hidden="true" className="w-8 h-8 text-brand-200 mb-4" />
              <p className="text-gray-700 mb-5 flex-1 leading-relaxed">«{it.quote}»</p>
              <p className="text-sm font-medium text-gray-500">{it.author}</p>
            </Card>
          </FadeIn>
        ))}
      </div>
