  ...
  return (
    <Section ariaLabelledby="usecases-heading">
      <FadeIn className="text-center mb-16 max-w-2xl mx-auto">
        ...
        <h2 id="usecases-heading" className="text-4xl md:text-5xl font-semibold tracking-tight text-gray-900 text-balance">
          {t('useCases.h2')}
        </h2>
      </FadeIn>
          ...
          return (
            <FadeIn key={it.title} delay={i * 100}>
              <Card className="p-6 h-full flex flex-col">
                <div className="inline-flex w-10 h-10 rounded-lg bg-brand-50 items-center justify-center mb-4">
                  <Icon aria-hidden="true" className="w-5 h-5 text-brand-700" />
                </div>
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{it.title}</h3>
                <p className="text-gray-600 mb-5 flex-1">{it.text}</p>
                <ul className="space-y-2">
                  {it.benefits.map((b) => (
                    <li key={b} className="flex items-start gap-2 text-sm text-gray-700">
                      <CheckCircle2 aria-hidden="true" className="w-4 h-4 text-brand-500 flex-shrink-0 mt-0.5" />
